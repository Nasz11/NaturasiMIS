...
/**
 * NaturasiMIS - Search Feature Setup Script
 * 
 * HOW TO USE:
 * 1. Put this file in your Laravel ROOT folder (same level as artisan)
 * 2. Open terminal in that folder
 * 3. Run: php setup_search.php
 * 4. Done! Then clear cache with: php artisan view:clear && php artisan cache:clear
 */

echo "\n===========================================\n";
echo "  NaturasiMIS Search Feature Setup\n";
echo "===========================================\n\n";

// ─── STEP 1: Create search views folder ───────────────────────────
$searchDir = __DIR__ . '/resources/views/search';
if (!is_dir($searchDir)) {
    mkdir($searchDir, 0755, true);
    echo "[✓] Created folder: resources/views/search/\n";
} else {
    echo "[~] Folder already exists: resources/views/search/\n";
}

// ─── STEP 2: Create search/index.blade.php ────────────────────────
$bladeFile = $searchDir . '/index.blade.php';
$bladeContent = <<<'BLADE'
@extends('layouts.app')
...
<section id="search-results">
  ...
  <div style="display:flex;gap:1rem;margin-bottom:1.5rem;flex-wrap:wrap;">
    <div style="background:#fff;border:1px solid #e0e0e0;border-radius:10px;padding:.6rem 1.2rem;font-size:.85rem;color:#555;">
      ...
      Production Batches: <strong>{{ $productionBatches->count() }}</strong>
    </div>
    <div style="background:#fff;border:1px solid #e0e0e0;border-radius:10px;padding:.6rem 1.2rem;font-size:.85rem;color:#555;">
    ...
    <div style="background:#fff;border:1px solid #e0e0e0;border-radius:10px;padding:.6rem 1.2rem;font-size:.85rem;color:#555;">
      ...
      Inventory: <strong>{{ $inventoryItems->count() }}</strong>
    </div>
  </div>

  {{-- NO RESULTS --}}
  @if($productionBatches->isEmpty() && $batches->isEmpty() && $inventoryItems->isEmpty())
  ...
  <div class="result-section">
    <h3 style="font-size:1rem;color:#0e472d;margin-bottom:.75rem;">
      <i class="fas fa-industry"></i> Production Batches ({{ $productionBatches->count() }})
    </h3>
    <div style="display:flex;flex-direction:column;gap:.6rem;">
      @foreach($productionBatches as $pb)
  ...
  <div class="result-section" style="margin-top:1.5rem;">
    <h3 style="font-size:1rem;color:#0e472d;margin-bottom:.75rem;">
      <i class="fas fa-boxes"></i> Inventory ({{ $inventoryItems->count() }})
    </h3>
    <div style="display:flex;flex-direction:column;gap:.6rem;">
...
echo "[✓] Created file: resources/views/search/index.blade.php\n";

// ─── STEP 3: Patch DashboardController ────────────────────────────
$controllerPath = __DIR__ . '/app/Http/Controllers/DashboardController.php';
$controllerContent = file_get_contents($controllerPath);

// Check if search method already exists
if (strpos($controllerContent, 'public function search') !== false) {
    echo "[~] search() method already exists in DashboardController — skipping.\n";
} else {
    // Add Request import if missing
    if (strpos($controllerContent, 'use Illuminate\Http\Request;') === false) {
    ...
    public function search(Request $request)
    {
        ...
        $productionBatches = ProductionBatch::with('staff')
        ...
        $batches = Batch::with('staff')
            ...
            ->orWhere('cheese_type', 'like', "%{$query}%")
            ->orWhere('status',      'like', "%{$query}%")
            ->orderByDesc('created_at')
            ->get();

        $inventoryItems = InventoryItem::with('updatedBy')
        ...
        return view('search.index', compact('query', 'productionBatches', 'batches', 'inventoryItems'));
    }
PHP;

    $controllerContent = preg_replace('/\}\s*$/', $searchMethod . "\n}", $controllerContent);
    file_put_contents($controllerPath, $controllerContent);
    echo "[✓] Added search() method to DashboardController.php\n";
}
...
echo "  Setup Complete!\n";
echo "===========================================\n";
echo "\nNow run these commands to clear cache:\n";
echo "  php artisan view:clear\n";
echo "  php artisan cache:clear\n\n";
echo "Then test it: go to your app and search for a batch!\n\n";
